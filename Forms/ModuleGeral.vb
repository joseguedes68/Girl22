...
Module ModuleGeral
    ...
    Sub main()
        ...
        Try
            ...
            Else
                ...
                MsgBox("Alterar as Configurações Regionais :" & Chr(13) & _
                   "Separador decimal: Ponto " & Chr(13) & _
                   "Símbolo de agrupamento de dígitos: Espaço " & Chr(13) & _
    ...
    Public Sub ErroVB(ByVal Mensagem As String, ByVal Local As String)
        MsgBox("Surgiu um erro no programa!" & Chr(13) & Chr(13) & _
               "Com Descrição:" & Mensagem & Chr(13) & Chr(13) & _
    ...
    Public Sub FecharEtiquetas(ByVal xDocFecho As String, ByVal gIdDocCabOrig As String)
        Try
            ...
            If db.GetDataValue(Sql) > 0 Then EnviarEmail("Erro", "Há " + db.GetDataValue(Sql) + " TALÃO(ÕES) ABERTOS DEPOIS DO FECHO: " + gIdDocCabOrig.ToString)
    ...
    Public Sub ExportToGirl()
        ...
        Try
            ...
            If dbCelfGest.GetDataValue(Sql) = "1" Then
                MsgBox("A Exportação está a ser executada por outro utilizador!")
                Exit Try
            End If
            ...
            For Each rRastreio As DataRow In dtRastreio.Rows
                ...
                Else
                    GravarEvento("Erro Nº101!!", xArmz, "00000000", "Exp")
    ...
    Public Sub ImportFromGirl()
        ...
        Try
            ...
            If dtRastreio.Rows.Count >= 3500 Then
                ...
                'If MsgBox("Tempo estimado de Importação : " & Int(dtRastreio.Rows.Count / 300) & " min", MsgBoxStyle.OkCancel) = MsgBoxResult.Cancel Then
                '    Exit Sub
                'End If
            End If
            ...
            For Each rRastreio As DataRow In dtRastreio.Rows
                        ...
                        Select Case rRastreio("ACCAO").ToString
                            ...
                            Case "UPDATE"
                                ...
                                If dr.HasRows Then
                                    ...
                                    If dr("EstadoID") <> "P" Then  'ENQUANTO NÃO APAGO O ESTADO P
                                        'SE ALGUEM MEXER EM INFORMAÇÃO NA TABELA SERIE DO SERVIDOR ANTES DO ESTADO P, VAI ALTERAR UM TALÃO QUE PODE ESTAR SEPARADO
                        ...
                        Select Case rRastreio("ACCAO").ToString
                            ...
                            Case "UPDATE"
                                ...
                                If dr.HasRows Then
                                    ...
                                    'VERIFICAR SE É UMA SEPARAÇÃO PARA A PRÓPRIA CASA 'D-DEFEITOS' E SE FOI FEITO O ENVIO!!!
                        ...
                        Select Case rRastreio("ACCAO").ToString
                            ...
                            Case "INSERT"
                                ...
                                If dr.HasRows Then
                                    ...
                                    'VERIFICAR SE É UMA SEPARAÇÃO PARA A PRÓPRIA CASA 'DEFEITOS'
                                    If dr("ArmazemID").ToString = dr("TercID").ToString And dr("TipoDocID") = "SE" Then
                                        ...
                                        If dberror = False Then
                                            'NOTA O UPDATE AO ESTADO DO TALAO AQUI NÃO VAI DESFAZER ISTO?? ( XPTO123456 )
                                            ...
                                            If dberror = True Then 'VAI GRAVAR O EVENTO NO CELFGEST
                                                'GravarEvento("ERRO 395 AO ACTUALIZAR TALÃO PARA V", dr("SerieID") + "-" + dr("ArmazemID").ToString, Now, "EMAIL")
                                                EnviarEmail("Erro", "ERRO 395 AO ACTUALIZAR TALÃO PARA V" + " " + dr("SerieID") + " " + dr("ArmazemID").ToString + "  " + Now)
                                                dberror = False
                                            End If
                                        End If
                ...
                If dberror = False Then
                    ...
                    If dberror = True Then
                        MsgBox("Erro Nº102! Avise o Escritório!")
                    End If
                    ''Else
                    '    ActualizaLigacao("CelfGest")
                    ...
                    '    dbDelRastreio.ExecuteData(Sql)
                Else
                    'GravarEvento("ERRO SQL - 396 NA IMPORTAÇÃO", , Now, "EMAIL")
                    EnviarEmail("Erro", "ERRO SQL - 396 NA IMPORTAÇÃO" + " " + Now)
                    ActualizaLigacao("Girl")
                    Sql = "UPDATE Rastreio SET Error = 'True' WHERE idRastreio='" & rRastreio("idRastreio").ToString & "'"
                    dbDelRastreio.ExecuteData(Sql)
                End If
        ...
        Finally
            ...
            If dberror = True Then GravarEvento("Erro Nº102!!", xArmz, "00000000", "xxxx")
            If dbRastreio IsNot Nothing Then dbRastreio.Dispose()
            If dbCelfGest IsNot Nothing Then dbCelfGest.Dispose()
        ...
        Try
            ...
            For Each r As DataRow In dtTalao.Rows
                ...
                If r("EstadoID").ToString <> "S" Then
                    ...
                    EnviarEmail("Erro", "ERRO 256 AO EXPORTAR O TALÃO " + Now)
                    If sAplicacao = "CELFGEST" Then ActualizaLigacao("CelfGest")
                    Exit Sub
                End If
                ...
                If dberror = True Then
                    ...
                    EnviarEmail("Erro", "ERRO NA GRAVAÇÃO DA EXPORTAÇÃO! 666 " + Now)
                    MsgBox("NÃO FOI POSSIVEL EXPORTAR O TALÃO", MsgBoxStyle.Information)
                End If
